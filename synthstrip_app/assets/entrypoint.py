...
from biomarkers.entrypoints import synthstrip, tapismpi
...
async def main(
    ...
    await synthstrip.SynthStripEntrypoint(
        outs=outdirs,
        ins=bids_directory,
        stage_ignore_patterns=shutil.ignore_patterns(
            "*.heudiconv", "sourcedata", "func", "fmap", "dwi"
        ),
    ).run()
