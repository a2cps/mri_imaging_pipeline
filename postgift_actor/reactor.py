...
class PostGIFTReactor(models.Reactor):
    def get_runlist(self) -> list[str]:  # type: ignore
        if "postgift" not in self.ilog.columns:
            ilog = self.ilog.with_columns(postgift=0)
        else:
            ilog = self.ilog
        rundef = (
            ilog
            # exclude rows that were already processed
            .filter(pl.col("postgift") == 0)
            .filter(pl.col("gift") == 1)
            .with_columns(
...
def main() -> None:
    PostGIFTReactor(
        job_name=f"postgift-{datetime.datetime.today().strftime('%Y-%m-%d')}",
