...
def run_and_print_sys_cmd(cmd: str):
    print(cmd)
    os.system(cmd)
...
def create_fmri_b0(b0_file):
    ...
    run_and_print_sys_cmd(f"fslsplit {b0_file} {os.path.join(basepath,'vol')}")
    ...
    run_and_print_sys_cmd(f"fslmerge -t {output_AP_fname} {os.path.join(basepath,'vol0001.nii.gz')} {os.path.join(basepath,'vol0003.nii.gz')}")
    ...
    run_and_print_sys_cmd(f"fslmerge -t {output_PA_fname} {os.path.join(basepath,'vol0000.nii.gz')} {os.path.join(basepath,'vol0002.nii.gz')}")

    # now that new niftis have been created, delete the files generated by fslsplit
    ...
    run_and_print_sys_cmd(f"fslorient -deleteorient {output_PA_fname}")

    # flip the orientation of AP and save as PA (to make them the same orientation)
    run_and_print_sys_cmd(f"fslswapdim {output_PA_fname} x -y z {output_PA_fname}")

    # copy back header information from correctly oriented AP
    run_and_print_sys_cmd(f"fslcpgeom {output_AP_fname} {output_PA_fname}")
