...
def print_and_run_sys_cmd(cmd: str) -> None:
    print(cmd)
    os.system(cmd)
...
def create_fmri_b0(b0_file):
    ...
    Note that the GE data were packaged as invalid dicoms, in which the AP and
    PA images were in the same series. For details, see confluence:
    https://confluence.a2cps.org/display/DOC/B0+Distortion+Correction+Issues
    section: "UIC FMRI_B0 Protocol Format" (applies to any GE dataset, not just UIC). 
    """

    # Get input path and subject name
    basepath = str(Path(b0_file).parents[0])

    # Split the fmri b0 image into four volumes (saved as niftis, deleted below)
    print_and_run_sys_cmd(f"fslsplit {b0_file} {os.path.join(basepath,'vol')}")
    ...
    print_and_run_sys_cmd(f"fslmerge -t {output_AP_fname} {os.path.join(basepath,'vol0001.nii.gz')} {os.path.join(basepath,'vol0003.nii.gz')}")
    ...
    print_and_run_sys_cmd(f"fslmerge -t {output_PA_fname} {os.path.join(basepath,'vol0000.nii.gz')} {os.path.join(basepath,'vol0002.nii.gz')}")

    # delete the files generated by fslsplit
    files_to_remove = glob.glob(os.path.join(basepath,"vol*"))
    for i in files_to_remove:
        os.remove(i)

    print("Saving PA image as %s"%output_PA_fname)
    print("Updating PA image orientation...")

    # remove orientation info from PA
    print_and_run_sys_cmd(f"fslorient -deleteorient {output_PA_fname}")

    # flip the orientation of AP and save as PA (to make them the same orientation)
    print_and_run_sys_cmd(f"fslswapdim {output_PA_fname} x -y z {output_PA_fname}")

    # copy back header information from correctly oriented AP
    print_and_run_sys_cmd(f"fslcpgeom {output_AP_fname} {output_PA_fname}")
