...
from biomarkers.entrypoints import postgift, tapismpi
from mpi4py import MPI

tapismpi.configure_mpi_logger()


async def main(ins: list[Path], outs: list[Path]) -> None:
    await postgift.PostGIFTEntrypoint(
        outs=outs,
        ins=ins,
        stage_ignore_patterns=shutil.ignore_patterns(
            "*bold.nii.gz", "network_summary", "*gica_results"
        ),
    ).run()
...
if __name__ == "__main__":
    ...
    asyncio.run(main(ins=args.input_dirs, outs=output_dirs))
