...
class BEDPOSTXReactor(models.Reactor):
    def get_runlist(self) -> list[str]:
        if "bedpostx" not in self.ilog.columns:
            ilog = self.ilog.with_columns(bedpostx=0)
        else:
            ilog = self.ilog
        rundef = (
            ilog
            # exclude rows that were already processed
            .filter(pl.col("bedpostx") == 0)
            ...
            .with_columns(
                INPUT_DIR=pl.concat_str(
                    ...
                    pl.col("sublong"),
                    pl.lit("/qsirecon-fsl/derivatives/qsirecon-FSL"),
                )
            )
            .sort(
...
def main() -> None:
    BEDPOSTXReactor(
        job_name=f"bedpostx-{datetime.datetime.today().strftime('%Y-%m-%d')}",
