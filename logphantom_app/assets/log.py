...
class Log:
    ...
    pem = "confluence-a2cps-org-chain.pem"
    ...
    def set_session(self, token):
        ...
        s.verify = self.pem
        self.session = Confluence(url=self.site, session=s)
    ...
    def merge_logs(self):
        ...
        newlog = self.processing.merge(
            self._scans, on=["site", "date"], how="left"
    ...
    def post_log(self):
        ...
        soup.table.replace_with(
            BeautifulSoup(self._newlog.to_html(index=False, na_rep=""), "html.parser")
        )
        # update page with new content
        self.session.update_page(
            page_id=self.pageid, title=self.pagetitle, body=str(soup)
        )
    ...
    def set_processing(self):
        ...
        for site in [
            ...
            bids += [
                z
                for z in (self.products / site / "bids").glob("*QC*")
                if len([x for x in z.glob("*out")]) > 0
            ]

        bids_df = pd.DataFrame({"bids": bids})
        bids_df["id"] = bids_df["bids"].apply(lambda x: x.stem)
        bids_df["bids"] = bids_df["bids"].apply(
            lambda x: datetime.datetime.fromtimestamp(x.stat().st_ctime).date()
        )
        ...
        dicoms_df["dicom"] = dicoms_df["dicom"].apply(
            lambda x: datetime.datetime.fromtimestamp(x.stat().st_ctime).date()
        )
    ...
    def set_current_scans(self):
        scans = pd.concat(
            [
                pd.read_csv(x, delim_whitespace=True)
                for x in self.bidsroot.glob("sub-*/ses*/*scans.tsv")
            ],
            ignore_index=True,
            sort=False,
        )
        ...
        self._scans = (
            scans.pivot(index=["site", "date"], columns="scan", values="received")
            .fillna("N")
    ...
    def _extract_site(filename: pd.Series):
        return filename.str.extract(
            r"(ns|ws|sh|ui|uc|um)", expand=False, flags=re.IGNORECASE
        ).str.upper()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=pathlib.Path(
            "/corral-secure/projects/A2CPS/resources/imaging/phantom/bids"
        ),
    )
    parser.add_argument(
        "--products",
    ...
    parser.add_argument(
        "--post", action=argparse.BooleanOptionalAction, default="--no-post"
    )

    args = parser.parse_args()
    main(bidsroot=args.bidsroot, products=args.products, post=args.post)
